...
void LoadBalancer::run(){
    ...
    while (true) {
        ...
        for (auto &srv : servers) {
            ...
            // If server is idle but queue is not empty, give it a request.
            else if (!srv.isBusy() && !requestQueue.empty()) {
                Request next = requestQueue.front();
                requestQueue.pop();
                srv.setRequest(next);
        ...
        for (size_t i = 0; i < servers.size(); i++) {
            printf("Server %zu: busy=%d, remaining_time=%zu\n", 
                   i, servers[i].isBusy(), servers[i].getRemainingTime());
        }

        // If all servers are idle and the queue is empty, we can stop.
        bool allIdle = true;
        for (auto &srv : servers) {
            if (srv.isBusy()) {
                allIdle = false;
        ...
        // If the runtime limit is reached, also stop.
        if (currentTime >= runTime) {
            cout << "Reached max runtime (" << runTime << "). Stopping. \n";
            break;
        }
