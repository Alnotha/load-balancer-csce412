...
void LoadBalancer::adjustServers() {
    // Scale up if queue size is large
    if (!requestQueue.empty() && activeServers < maxServers) {
        servers.emplace_back("Server" + std::to_string(activeServers));
        activeServers++;
        std::cout << "Scaled up to " << activeServers << " servers.\n";
    }

    // Check idle servers and scale down
    int idleCount = 0;
    ...
    if (idleCount > 2 && activeServers > minServers) { // Example threshold
        servers.pop_back();
        activeServers--;
        std::cout << "Scaled down to " << activeServers << " servers.\n";
    }
}
